    ...
    try {
      ...
      if (allowance > 0n) {
        const rawData = await this.walletProvider.approveTx({
          ...
          spender: CHAIN[this.walletProvider.chain()].EARN_ADDRESS,
          amount: 0
        });
      ...
      const r = await this.walletProvider.earnTx({ from: this.walletProvider.address(), amount: value.amount, index: value.period });
