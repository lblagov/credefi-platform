...
export class UtilitiesComponent extends ConnectDialog {
  ...
  async setTier() {
    const tierLevel = await this.wallet.getTierLevel();
    this.tierLevel.set(tierLevel);
  }


  async setLocked() {
    const data = await this.wallet.getUtilityAmount();
    this.locked.set(data.amount);
  }
  ...
  async onSubmitMetamask() {
    try {
      ...
      const rawData = await this.wallet.approveXCrediTx({
        ...
        spender: CHAIN[this.wallet.chain()].UTILITY_ADDRESS,
        amount: value.amount
      });
      ...
      const r = await this.wallet.utilizeTX({ from: this.wallet.address(), amount: value.amount });
      delete r.nonce;
  ...
  async onClaimMetamask() {
    try {
      ...
      const r = await this.wallet.onClaimUtility({ from: this.wallet.address() });
      delete r.nonce;
