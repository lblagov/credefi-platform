...
import { addDays, addMonths, addYears, format, getDate, getDay, getDaysInMonth, getMonth, getYear, Locale, parse, parseISO, setDay, setMonth, toDate, parseJSON } from 'date-fns';
...
export const DATEFNS_DATE_ADAPTER_OPTIONS =
  new InjectionToken<NgxDateFnsDateAdapterOptions>(
    'DATEFNS_DATE_ADAPTER_OPTIONS',
    {
      providedIn: 'root',
      factory: DATEFNS_DATE_ADAPTER_OPTIONS_FACTORY,
    }
  );

export function DATEFNS_DATE_ADAPTER_OPTIONS_FACTORY(): NgxDateFnsDateAdapterOptions {
  return {
    useUtc: false,
  };
}
...
export class DateFnsDateAdapter extends DateAdapter<Date> {
  ...
  private getLocale = (localeCodeOrLocale: string | Locale): Locale => {
    ...
    if (!this.locales || !this.locales.length) {
      throw new Error('locales array does not provided or is empty. Provide it via the DATEFNS_LOCALES token.');
    }
    const locale = this.locales.find(
      (item) => item.code === localeCodeOrLocale
    );
    if (!locale) {
      throw new Error(`locale '${localeCodeOrLocale}' does not exist in locales array. Add it to the DATEFNS_LOCALES token.`);
    }
    return locale;
  };
  ...
  constructor(
    ...
    @Optional() @Inject(DATEFNS_LOCALES) private locales: Locale[] | null,
    @Optional()
    @Inject(DATEFNS_LOCALES)
  ...
  deserialize(value: string | number | Date): Date | null {
    if (value) {
  ...
  isDateInstance(obj: Date): boolean {
  ...
  parse(value: string | Date, parseFormat: string): Date | null {
    if (value) {
      if (typeof value === 'string') {
        if (this.options?.useUtc) {
  ...
  toUTC(date: Date) {
    ...
    return new Date(format(d, 'LLL dd, yyyy HH:mm'));
  }
  ...
  private _createDateInternal(year: number, month: number, date: number): Date {
    if (this.options?.useUtc) {
      return this.toUTC(new Date(year, month, date));
    }
